...
Usage:
  ./scripts/publish-wasm-oci.sh --owner <github-owner> [--tag <tag>] [--crate-dir <path>] [--no-build]

Options:
  --owner <owner>        GitHub owner/user/org for ghcr.io/<owner>/mdcs-wasm (required)
...
echo "$TOKEN" | wkg oci login ghcr.io --username "$OWNER" --password-stdin

echo "==> Pushing OCI artifact: $OCI_REF"
wkg oci push "$OCI_REF" "$WASM_PATH"

echo "==> Pulling back for verification"
TMP_PULL="$(mktemp -t mdcs-wasm-oci-XXXXXX.wasm)"
wkg oci pull "$OCI_REF" -o "$TMP_PULL"
