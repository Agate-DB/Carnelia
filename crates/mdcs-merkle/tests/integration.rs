...
use mdcs_merkle::{
    MerkleNode, Payload, NodeBuilder, Hasher,
    DAGStore, MemoryDAGStore,
    DAGSyncer, SyncResponse, SyncSimulator,
    BroadcastNetwork,
};
...
fn test_bootstrap_from_root_cid() {
    ...
    let all_nodes: Vec<MerkleNode> = source_syncer.store()
        .topological_order()
        .iter()
    ...
    let stored = new_syncer.apply_response(SyncResponse::with_nodes(all_nodes)).unwrap();
...
fn test_gap_repair() {
    ...
    target_syncer.apply_nodes_unchecked(vec![node_d.clone()]).unwrap();
    
    // Should have missing nodes
    assert!(!target_syncer.store().missing_nodes().is_empty());
    
    // Add more nodes
    target_syncer.apply_nodes_unchecked(vec![node_b.clone()]).unwrap();
    target_syncer.apply_nodes_unchecked(vec![node_c.clone()]).unwrap();
    target_syncer.apply_nodes_unchecked(vec![node_a.clone()]).unwrap();
...
fn test_tampered_node_rejected() {
    ...
    assert!(matches!(result, Err(mdcs_merkle::DAGError::VerificationFailed(_))));
}
