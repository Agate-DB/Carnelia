...
use crate::store::{DAGStore, DAGError};
...
impl<S: DAGStore> DAGSyncer<S> {
                ...
                if !peer_has.contains(&cid) {
                    if let Some(node) = self.store.get(&cid) {
                        if nodes.len() < limit {
                            ...
                            let has_parents = node.parents.iter()
                                .all(|p| peer_has.contains(p) || 
                                    nodes.iter().any(|n| n.cid == *p));
    ...
    pub fn find_missing_ancestors(&self, cids: &[Hash]) -> Vec<Hash> {
        ...
        let mut queue: VecDeque<(Hash, usize)> = cids.iter()
            .map(|cid| (*cid, 0))
            .collect();
...
impl SyncSimulator {
    ...
    pub fn new(n: usize) -> Self {
        let syncers = (0..n)
            .map(|i| {
                let (store, _) = crate::store::MemoryDAGStore::with_genesis(format!("replica_{}", i));
                DAGSyncer::new(store)
            })
            .collect();
