...
impl Awareness {
    ...
    pub fn get_users(&self) -> Vec<UserPresenceInfo> {
        ...
        tracker.all_users().map(|presence| {
            let cursors: HashMap<String, CursorInfo> = presence.cursors.iter().map(|(doc_id, cursor): (&String, &Cursor)| {
                let (sel_start, sel_end) = cursor.selection_range()
                    .map(|(s, e)| (Some(s), Some(e)))
                    .unwrap_or((None, None));
                (doc_id.clone(), CursorInfo {
